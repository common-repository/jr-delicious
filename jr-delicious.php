<?php
/*
Plugin Name: JR Delicious
Plugin URI: http://www.jakeruston.co.uk/2010/02/wordpress-plugin-jr-delicious/
Description: This plugin allows you to show your latest Delicious bookmarks as a widget.
Version: 1.2.2
Author: Jake Ruston
...
$pluginname="delicious";

// Hook for adding admin menus
add_action('admin_menu', 'jr_Delicious_add_pages');

// action function for above hook
function jr_Delicious_add_pages() {
    add_options_page('JR Delicious', 'JR Delicious', 'administrator', 'jr_Delicious', 'jr_Delicious_options_page');
}
...
register_activation_hook(__FILE__,'delicious_choice');

if (!function_exists("jr_delicious_refresh")) {
function jr_delicious_refresh() {
update_option("jr_submitted_delicious", "0");
}
}

function delicious_choice () {
if (get_option("jr_delicious_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_delicious";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_delicious", "1");
wp_schedule_single_event(time()+172800, 'jr_delicious_refresh'); 
} else {
...
update_option("jr_delicious_links_choice", $content);
}
}

if (get_option("jr_delicious_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_delicious_link_personal", $content);
}
}

// jr_Delicious_options_page() displays the page content for the Test Options submenu
function jr_Delicious_options_page() {

    // variables for the field and option names 
    $opt_name = 'mt_Delicious_header';
	$opt_name_2 = 'mt_Delicious_color';
    $opt_name_3 = 'mt_Delicious_account';
	$opt_name_4 = 'mt_Delicious_number';
    $opt_name_6 = 'mt_Delicious_plugin_support';
    $hidden_field_name = 'mt_Delicious_submit_hidden';
    $data_field_name = 'mt_Delicious_header';
	$data_field_name_2 = 'mt_Delicious_color';
    $data_field_name_3 = 'mt_Delicious_account';
	$data_field_name_4 = 'mt_Delicious_number';
    $data_field_name_6 = 'mt_Delicious_plugin_support';
...
<?php
    ...
    echo "<h2>" . __( 'JR Delicious Plugin Options', 'mt_trans_domain' ) . "</h2>";
	$blog_url_feedback=get_bloginfo('url');
...
<?php
    ...
    $change4 = get_option("mt_Delicious_plugin_support");
...
function show_Delicious($args) {

extract($args);

  $Delicious_header = get_option("mt_Delicious_header"); 
  $plugin_support2 = get_option("mt_Delicious_plugin_support");
  $Delicious_type = get_option("mt_Delicious_account");
  $Delicious_number = get_option("mt_Delicious_number");
  $Deliciouscolor = get_option("mt_Delicious_color");


if ($Delicious_header=="") {
$Delicious_header="Latest Delicious Bookmarks";
}
...
if ($Delicious_type=="") {
$Delicious_type="jakeruston";
}

$i=1;

echo $before_title.$Delicious_header.$after_title.$before_widget; 

echo "<ul>";
$doc='http://feeds.delicious.com/v2/rss/'.$Delicious_type;
$doc=curl_get_contents($doc) or die("Error!");
$xml=new SimpleXMLElement($doc);

foreach ($xml->channel->item as $node) {
$DeliciousTitle = $node->title;
$DeliciousURL = $node->link;

if ($DeliciousTitle != "") {
echo "<li style='color:#".$Deliciouscolour."'><a href='".$DeliciousURL."'>".$DeliciousTitle."</a></li>";
}

if($i++ >= $Delicious_number) break;

}
echo "</ul>";

if ($plugin_support2=="Yes" || $plugin_support2=="") {
$linkper=utf8_decode(get_option('jr_delicious_link_personal'));

if (get_option("jr_Delicious_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_Delicious_links_choice'));
...
update_option("jr_Delicious_links_choice", $new);
update_option("jr_Delicious_link_newcheck", "444");
}

if (get_option("jr_submitted_delicious")=="0") {
$pname="jr_delicious";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_delicious", "1");
...
wp_schedule_single_event(time()+172800, 'jr_delicious_refresh'); 
} else if (get_option("jr_submitted_delicious")=="") {
$pname="jr_delicious";
$url=get_bloginfo('url');
$current=get_option("jr_delicious_links_choice");
...
echo "<p style='color: #".$Deliciouscolor."; font-size:x-small'>Delicious Plugin created by ".$linkper." - ".stripslashes(get_option('jr_Delicious_links_choice'))."</p>";
}
...
function init_Delicious_widget() {
register_sidebar_widget("JR Delicious", "show_Delicious");
}

add_action("plugins_loaded", "init_Delicious_widget");
